      ...
      if (syndicateAddress && syndicate) {
        ...
        await memberAddresses.forEach(async (memberAddress) => {
          ...
          distributionTokens.forEach(async (distributionToken) => {
            ...
            if (syndicate.distributing) {
              ...
              if (memberDistributionsWithdrawalEvents.length) {
                ...
                for (
                  ...
                  const { amount } =
                    memberDistributionsWithdrawalEvents[i].returnValues;
                ...
                memberDistributionsWithdrawalsToDate =
                  tokenWithdrawalAmounts.toString();
              }
            }
            ...
            try {
              eligibleWithdrawal =
                await DistributionLogicContract.calculateEligibleDistribution(
                  totalMemberDeposits,
            ...
            const withdrawalsToDate = new BN(
              memberDistributionsWithdrawalsToDate,
            )
              .mul(new BN("100"))
              .toString();

            const memberWithdrawalsToDistributionsPercentage =
              divideIfNotByZero(
                withdrawalsToDate,
