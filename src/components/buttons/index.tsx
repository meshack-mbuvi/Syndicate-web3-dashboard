...
export const PrimaryButton = (props: {
  ...
  return (
    <button
      ...
      {createSyndicate ? (
        <img className="inline w-4 mr-4" src={`/images/${textColor === "text-white" ? "plus-circle-white" : "plus-circle"}.svg`} alt=""/>
      ) : null}
      {children}
    </button>
  );
...
export const SocialLinkButton  = (props: {
  ...
  const hoverColor = 
      source === SocialLinkSource.TWITTER ? "hover:bg-twitter-blue hover:bg-opacity-100"
    : source === SocialLinkSource.DISCORD ? "hover:bg-discord-purple hover:bg-opacity-100"
    : source === SocialLinkSource.TELEGRAM ? "hover:bg-telegram-blue hover:bg-opacity-100" 
    : source === SocialLinkSource.WEB ? "hover:bg-white hover:bg-opacity-50" : ""
...
export const SocialActionButton  = (props: {
  ...
  switch(action) {
    ...
    case SocialAction.MESSAGE:
      styles += " " + "bg-white bg-opacity-10 hover:bg-opacity-30 active:bg-opacity-10"
      label = "Message"
...
export const SocialButton = (props: {
  ...
  var styles = customClasses + " " + "py-social-action-button-small sm:py-social-action-button-large overflow-visible focus:outline-none min-h-9 sm:w-auto inline-flex justify-center items-center px-5 sm:px-8 mr-2 sm:mr-3 mb-2 transition duration-500 ease-out text-sm font-medium rounded-md text-white active:transition-none"
  const img = <img className={`${isLabelVisibleOnMobile ? "mr-1" : "sm:mr-2"} sm:mr-2`} src={icon}/>
  const text = <p className={`${isLabelVisibleOnMobile ? "" : "hidden sm:block"} font-whyte`}>{label}</p>
