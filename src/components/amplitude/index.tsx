...
export const initializeAmplitudeJS = (): void => {
  const isDeployPreview = window?.location?.hostname.indexOf('deploy-preview') > -1;
  if (window === undefined) {
    // An edge case when window is undefined
    return;
  } else if (window !== undefined && process.env.NODE_ENV === 'production' && !isDeployPreview) {
    // Initialize AmplitudeJS
    require('amplitude-js').getInstance().init(process.env.NEXT_PUBLIC_AMPLITUDE_API_KEY)
  } else {
    return;
  }
}
...
export const amplitudeLogger = (eventName: string, eventProperties: EventProperty): Promise<boolean> => {
  ...
  return new Promise(resolve => {
    if (window !== undefined && process.env.NODE_ENV === 'production' && !isDeployPreview) {
