...
export const DetailedTile: React.FC<Props> = ({
  ...
  const [highlightDimensions, setHighlightDimensions] = useState({x: 0, y: 0, height: 0});
  const [highlightTransitionStyles, setHighlightTransitionStyles] = useState("");
  ...
  const responsivelyCalculatedButtonPadding = `${width <= mobileBreakpoint && 'pt-4'}`;
  const responsivelyCalculatedSpaceBetween = `${width <= mobileBreakpoint && 'space-y-4'}`;
  const responsivelyCalculatedFlex = `${width <= mobileBreakpoint && 'flex-col'}`;
  ...
  const calculateHighlightDimensions = (buttonIndex: number) => {
    ...
    const buttonHeight = buttonRefs.current[buttonIndex].getBoundingClientRect().height;
      ...
      >
        <div
          ...
          style={{
              ...
              left: `calc(${highlightDimensions.x}px - ${width > mobileBreakpoint ? "0rem" : "1rem"})`,
              top: `calc(${highlightDimensions.y}px - ${width > mobileBreakpoint ? "1rem" : "0rem"})`,
              height: `calc(${highlightDimensions.height}px + ${width > mobileBreakpoint ? "2rem" : "1rem"})`,
              pointerEvents: "none"
            }}
        />
      </div>
